        ...
        if (args[0].toLowerCase() == "add") {
          ...
          if (!newPrefix) {
            return message.channel.send(
              `**${message.author.username}**, Please supply a new prefix! • (E.G) \`${data.Prefix}prefix add !\` `
            );
          }
          ...
          if (data) {
            try {
              ...
              newData.save().then(() => {
                message.channel.send(
                  `**${message.author.username}**, Set prefix to \`${newPrefix}\``
                );
              });
            } catch (err) {
              message.channel.send(
                `**${message.author.username}**, Error occured. This has been reported to the developer`
              );
              console.log(err);
            }
          } else if (!data) {
            try {
              ...
              newData.save().then(() => {
                message.channel.send(
                  `**${message.author.username}**, Updated prefix to \`${newPrefix}\``
                );
              });
            } catch (err) {
              message.channel.send(
        ...
        if (args[0].toLowerCase() == "remove") {
          if (data) {
            await prefixSchema
              ...
              .then(() => {
                message.channel.send(
                  `**${message.author.username}**, i have reset your prefix`
                );
              });
          } else if (!data) {
            message.channel.send(
              `**${message.author.username}**, There was no data found for this server!`
            );
          }
        }
      } else {

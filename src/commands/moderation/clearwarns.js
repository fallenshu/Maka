...
module.exports = {
    ...
    async execute(message, client, args, p) {
        ...
        if(data) {
            message.channel.send(`**${message.author.username}**, Deleted ${data.Warns.length} warn(s) from **${user.tag}**.`)
            ...
            message.channel.send(`**${message.author.username}**, Cant find any warns for this user.`)
        }
    } else {

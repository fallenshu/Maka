...
module.exports = {
  ...
  async execute(message, client, args, p, m) {
    ...
    if (message.member.hasPermission("MANAGE_CHANNELS" || "ADMINISTRATOR")) {
      ...
      if (!time) {
        return message.channel.send(IncorrctU).then((msg) => {
      ...
      if (ms(args[0]) > 21600) {
        const no = new MessageEmbed()
          ...
          .setDescription(
            `Slowmode must be less than \`7200000 ms\` (2 hours, 120 mins, 7200s) `
          )
          .setColor(main);
      ...
      const noPermis = new MessageEmbed()
        ...
        .setDescription(
          `You must have the \`MANAGE_CHANNELS\` permission to run this command`
        )
        .setColor(`E6E6FA`);
